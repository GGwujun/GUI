  ...
  <div>
    <scroller lock-x scrollbar-y use-pullup use-pulldown height="-50" @on-pullup-loading="onLoadMore" @on-pulldown-loading="onRefresh" v-model="status" ref="scroller">
          ...
          <router-link :to="{name: 'detail', query: { id: i.id }}">
            <div class="feed-title">
              <div class="feed-label" :class="[i.top ? 'feed-label-top' : `feed-label-other`]">
                {{i.tabs}}
                <!--//i.tabs | translateTab(i.top-->
              </div>
              <p v-text="i.title"></p>
            </div>
            <div class="feed-content">
              <router-link :to="{name: 'user', query: { id: i.loginname }}">
                <div class="avatar">
                  <img v-lazy="i.avatar_url" alt="headImgUrl">
                </div>
              </router-link>
              <div class="feed-right">
                <div class="feed-right-top">
                  <div class="feed-name" v-text="i.loginname">
                  </div>
                ...
                <div class="feed-right-bottom">
                  <div class="feed-time">
                    创建于：
                    <span>{{i.create_time | formatDate('yyyy-MM-dd hh:mm:ss')}}</span>
                  </div>
                  <div class="feed-pass">
      ...
      <div slot="pullup" class="xs-plugin-pullup-container xs-plugin-pullup-up" style="position: absolute; width: 100%; height: 40px; bottom: -40px; text-align: center;">
        <span v-show="status.pullupStatus === 'loading'" style="line-height: 40px;">
...
export default {
  ...
  methods: {
    onRefresh() {
      this.$loading.show()
      this.$axios.get('/Topics/selectArticle', {
        params: {
          page: 1,
          limit: 20
        }
